...
export function buildAgentMemoryContext({
  ...
  const openingRule =
    mode === "mean"
      ? stance === "agree"
        ? "用户在赞同上一轮 AI 判决。回复必须用“你看得很清”开头，并继续找 RC 专辑评价证据加码。"
        : stance === "disagree"
          ? "用户在反驳上一轮 AI 判决。回复必须用“xx迷不要胡搅蛮缠”开头，xx 用输家粉丝黑称；不要轻易改判，优先用 RC 专辑评价和对方短板反击。"
...
- 用户当前立场：${stance}
- 回复策略：${openingRule}
- 如果用户围绕乐评反驳，优先检查左侧采集数据里的 RC 专辑评分、评价体系和具体专辑证据。`;
}
